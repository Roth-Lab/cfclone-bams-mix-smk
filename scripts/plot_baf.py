...
def main(args):
    ...
    df["baf"] = df["ref_count"] / (df["ref_count"] + df["alt_count"])

    df["baf_phased"] = df["allele_0_count"] / (df["allele_0_count"] + df["allele_1_count"])

    df.loc[~df["valid"], ["baf", "baf_phased"]] = np.nan
...
def plot_by_chrom(df, chroms, fig, grid, title=None, y_col="baf"):
    ...
    y_min = max(y_min, df[y_col].quantile(0.01))

    y_max = min(y_max, df[y_col].quantile(0.99))
